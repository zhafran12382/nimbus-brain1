 ...
 *
 * Reschedules a task by updating its cron expression and run_at
 * in Supabase. The VPS worker will pick it up on the next poll.
 *
...
export async function POST(request: NextRequest) {
  ...
  if (
    [year, month, day, hour, minute].some((v) => v == null || isNaN(v)) ||
  ...
  const runAt = new Date(year, month - 1, day, hour, minute, 0, 0).toISOString();
  logger.scheduler('Parsed cron expression', { task_id, cronExpression, runAt }, correlationId);
  ...
  logger.scheduler('Task rescheduled successfully', { task_id: task.id, task_name: task.name, cronExpression, runAt, durationMs: Date.now() - start }, correlationId);
