...
const toolLabels: Record<string, string> = {
  ...
  run_python: "🐍 Running calculation...",
};
...
VERIFIKASI NUMERIK:
- Jika jawaban melibatkan angka, perhitungan, atau klaim kuantitatif dari web search:
  gunakan tool run_python untuk memverifikasi keakuratan angka tersebut.
- Contoh: jika sumber bilang "model 600B parameter butuh 1.2TB memori",
  verifikasi dengan run_python: print(600e9 * 2 / 1e12)  # FP16 = 2 byte per param
- Ini membantu mendeteksi typo atau kesalahan di artikel sumber.
- HANYA lakukan verifikasi jika ada angka spesifik yang bisa dihitung, bukan untuk semua search.
...
function getModeInstruction(mode: string): string {
  switch (mode) {
    case 'search':
      return '\n\n[MODE: SEARCH]\nDalam mode ini, SELALU gunakan web_search atau get_information terlebih dahulu sebelum menjawab pertanyaan faktual. Skip search HANYA untuk sapaan ringan atau perintah tool (buat target, catat expense, dll).\n\nSTRATEGI SEARCH WAJIB:\n- Lakukan MINIMAL 2 kali web_search dengan query BERBEDA untuk setiap topik.\n- Variasikan query: gunakan sinonim, bahasa berbeda (ID + EN), dan phrasing alternatif.\n\nALUR WAJIB SETELAH MENDAPAT HASIL SEARCH:\n1. KUMPULKAN: Jalankan web_search minimal 2x dengan query berbeda\n2. ANALISIS KRITIS: Setelah mendapat semua hasil search, JANGAN langsung percaya. Evaluasi:\n   - Apakah sumber kredibel? (media resmi vs blog random)\n   - Apakah informasi konsisten antar sumber?\n   - Apakah ada klaim yang kontradiktif?\n   - Apakah ada angka/statistik yang perlu diverifikasi?\n3. VERIFIKASI ANGKA (JIKA ADA): Jika jawaban melibatkan:\n   - Perhitungan matematis (harga × kuantitas, konversi satuan, persentase)\n   - Klaim numerik dari sumber (RAM, storage, bandwidth, dll)\n   - Perbandingan angka antar sumber\n   → WAJIB gunakan run_python untuk verifikasi. Contoh:\n   - Sumber bilang "600B parameter × 2 byte = 1.2TB" → verifikasi: run_python("print(600e9 * 2 / 1e12)")\n   - Sumber bilang "harga $500 = Rp 8 juta" → verifikasi: run_python("print(500 * 16000)")\n4. JAWAB DENGAN TINGKAT KEYAKINAN:\n   - Fakta yang konsisten di banyak sumber: nyatakan langsung\n   - Fakta yang hanya dari 1 sumber: gunakan "menurut [sumber]..."\n   - Angka yang sudah diverifikasi via Python: nyatakan dengan confidence\n   - Informasi yang kontradiktif: tampilkan SEMUA versi dengan sumbernya\n   - Informasi yang tidak ditemukan: jujur bilang tidak tahu\n\nATURAN KRITIS:\n- JANGAN pernah mengarang fakta, angka, nama, lokasi, atau kutipan\n- JANGAN gabungkan info dari sumber berbeda menjadi kesimpulan baru tanpa bukti\n- Gunakan "diduga", "menurut laporan", "berdasarkan sumber" untuk info belum pasti\n- Sebut sumber secara natural ("menurut Kompas...", "dari artikel Reuters...")\n- Jika hasil search kosong atau tidak relevan: jawab jujur "gue ga nemu info valid soal ini"\n\nFORMAT OUTPUT:\n- Ringkasan singkat di awal (2-4 kalimat)\n- Detail lengkap dengan sumber\n- Jika ada angka yang diverifikasi, tunjukkan proses verifikasinya\n- Minimal 200-400 kata untuk topik kompleks\n[/MODE]';
    case 'think':
